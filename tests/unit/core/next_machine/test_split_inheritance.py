"""Tests for Task 7: split_todo inherits parent approved prepare phase."""
...
# ---------------------------------------------------------------------------
# Test: no inheritance when parent has no approved phase
# ---------------------------------------------------------------------------
...
def test_split_no_approval_children_start_at_discovery(
    ...
    # Parent has no approved verdicts
    ...
    # No inherited phase — prepare_phase absent or empty
...
# ---------------------------------------------------------------------------
# Test: requirements approved → children inherit plan_drafting phase
# ---------------------------------------------------------------------------
...
def test_split_req_approved_children_inherit_plan_drafting(
    ...
    project_root, parent_slug = _make_parent(tmp_path)
    # Write parent requirements artifact
    ...
    # requirements.md should be copied (overwriting template)
...
# ---------------------------------------------------------------------------
# Test: plan approved → children inherit prepared phase
# ---------------------------------------------------------------------------
...
def test_split_plan_approved_children_inherit_prepared(
    ...
    # Both artifacts should be copied
...
# ---------------------------------------------------------------------------
# Test: skipped phases have status="skipped" audit entries
# ---------------------------------------------------------------------------


@patch("teleclaude.core.next_machine.core._emit_prepare_event")
@patch("teleclaude.todo_scaffold._emit_prepare_event", create=True)
def test_split_skipped_phases_have_audit_entries(
    mock_scaffold_emit: MagicMock,
    mock_core_emit: MagicMock,
    tmp_path: Path,
) -> None:
    """Children with inherited approval have audit entries for skipped phases."""
    from teleclaude.todo_scaffold import split_todo
    ...
    with patch("teleclaude.core.next_machine.core.add_to_roadmap", return_value=None):
        split_todo(project_root, parent_slug, ["child-d"])

    child_state = _read_child_state(tmp_path, "child-d")
    audit = child_state.get("audit", {})
    assert isinstance(audit, dict), f"expected audit dict, got {audit!r}"
    # requirements_review was skipped (inherited approve)
    rr_audit = audit.get("requirements_review", {})
    assert isinstance(rr_audit, dict)
    assert rr_audit.get("status") == "skipped", (
        f"expected status=skipped in requirements_review audit, got {rr_audit!r}"
    )
    assert rr_audit.get("reason") == "inherited_from_parent"


# ---------------------------------------------------------------------------
# Test: split_inherited event emitted per child
# ---------------------------------------------------------------------------


@patch("teleclaude.core.next_machine.core._emit_prepare_event")
def test_split_emits_split_inherited_event(
    mock_core_emit: MagicMock,
    tmp_path: Path,
) -> None:
    """split_todo emits prepare.split_inherited event for each child."""
    from teleclaude.todo_scaffold import split_todo
    ...
    with patch("teleclaude.core.next_machine.core.add_to_roadmap", return_value=None):
        with patch("teleclaude.todo_scaffold._emit_prepare_event") as mock_emit:
            split_todo(project_root, parent_slug, ["child-e"])

    # split_inherited should have been emitted for the child
    all_calls = [c[0][0] for c in mock_emit.call_args_list]
    assert any("split_inherited" in call for call in all_calls), (
        f"Expected split_inherited event, got: {all_calls}"
    )
