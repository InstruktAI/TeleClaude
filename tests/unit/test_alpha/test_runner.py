"""Unit tests for teleclaude_events.alpha.runner."""
...
from teleclaude_events.alpha.runner import (
    AlphaRunner,
...
# ---------------------------------------------------------------------------
# AlphaRunner._handle_client (via socket pair)
# ---------------------------------------------------------------------------
...
async def _run_single_request(
    ...
    """Spin up AlphaRunner, send one request, return the raw response dict."""
    sock_path = str(Path(tmpdir) / "test.sock")

    shutdown = asyncio.Event()
    runner = AlphaRunner(socket_path=sock_path, cartridges_dir=tmpdir)
...
async def test_ping_handler_returns_ok():
    with tempfile.TemporaryDirectory() as tmpdir:
        req = request_to_dict(
            AlphaRequest(cartridge_name="__ping__", envelope={}, catalog_snapshot=[])
        )
...
async def test_handle_client_timeout_returns_error():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("teleclaude_events.alpha.runner._CARTRIDGE_TIMEOUT", 0.05):
            resp = await _run_single_request(tmpdir, req)
