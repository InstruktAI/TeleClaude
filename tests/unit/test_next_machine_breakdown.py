...
async def test_next_prepare_hitl_input_md_unassessed_breakdown():
    ...
    with (
        ...
        assert "update state.json and create breakdown.md" in result
...
def test_read_breakdown_state_returns_defaults_when_no_file():
    """read_breakdown_state returns default breakdown state when state.json doesn't exist."""
...
def test_read_breakdown_state_returns_breakdown_when_exists():
    """read_breakdown_state returns breakdown dict from existing state.json."""
    with tempfile.TemporaryDirectory() as tmpdir:
        slug = "test-slug"
        ...
        state_file.write_text(
            json.dumps({"build": "pending", "breakdown": {"assessed": True, "todos": ["test-1", "test-2"]}})
        )
...
def test_read_breakdown_state_returns_defaults_when_breakdown_missing():
    """read_breakdown_state returns defaults when state.json exists but has no breakdown key."""
    with tempfile.TemporaryDirectory() as tmpdir:
        slug = "test-slug"
        state_dir = Path(tmpdir) / "todos" / slug
        state_dir.mkdir(parents=True)
        state_file = state_dir / "state.json"
        state_file.write_text(json.dumps({"build": "pending"}))
...
def test_write_breakdown_state_creates_breakdown_section():
    """write_breakdown_state creates breakdown section in state.json."""
    with tempfile.TemporaryDirectory() as tmpdir:
        slug = "test-slug"
