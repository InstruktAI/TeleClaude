"""Characterization tests for teleclaude.events.delivery.telegram."""
...
def _make_adapter(send_fn: AsyncMock, min_level: int = int(EventLevel.WORKFLOW)) -> TelegramDeliveryAdapter:
    return TelegramDeliveryAdapter(chat_id="chat-456", send_fn=send_fn, min_level=min_level)
...
async def test_on_notification_sends_when_created_and_above_min_level() -> None:
    ...
    assert call_kwargs["chat_id"] == "chat-456"
...
async def test_on_notification_swallows_send_exceptions() -> None:
    send_fn = AsyncMock(side_effect=RuntimeError("telegram down"))
    adapter = _make_adapter(send_fn)
    await adapter.on_notification(
        notification_id=4,
        event_type="content.published",
...
async def test_default_min_level_is_workflow() -> None:
    ...
    adapter = TelegramDeliveryAdapter(chat_id="chat", send_fn=send_fn)
    await adapter.on_notification(
        notification_id=5,
        event_type="event",
