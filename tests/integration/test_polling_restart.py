...
@pytest.mark.asyncio
async def test_polling_restarts_after_process_exits(tmp_path):
    ...
    """
    # Setup: Initialize DB with temp file
    test_db_path = tmp_path / "test.db"
    Path(test_db_path).unlink(missing_ok=True)
    test_db = Db(str(test_db_path))
...
@pytest.mark.asyncio
async def test_polling_guard_prevents_duplicate_polling(tmp_path):
