"""Acceptance and regression tests for integrator cutover guardrails."""
...
def _base_env(tmp_path: Path, wrapper_dir: Path, fake_bin: Path) -> dict[str, str]:
    ...
    env["TELECLAUDE_SESSION_ID"] = "sess-cutover"
    env["TELECLAUDE_INTEGRATOR_CUTOVER_ENABLED"] = "1"
    env["TELECLAUDE_INTEGRATOR_PARITY_EVIDENCE"] = "accepted"
    return env


@pytest.mark.integration
@pytest.mark.timeout(5)
def test_non_integrator_canonical_main_push_is_rejected(tmp_path: Path) -> None:
    ...
    env["TELECLAUDE_SYSTEM_ROLE"] = "worker"
    ...
    assert "INTEGRATOR_MAIN_AUTH_BLOCKED" in result.stderr
    assert "GUARDRAIL_MARKER" in result.stderr


@pytest.mark.integration
@pytest.mark.timeout(5)
def test_integrator_canonical_main_push_still_succeeds(tmp_path: Path) -> None:
    canonical_root = tmp_path / "repo"
    (canonical_root / ".git").mkdir(parents=True, exist_ok=True)
    ...
    env["FAKE_GIT_BRANCH"] = "main"
    env["FAKE_GIT_DIR"] = str(canonical_root / ".git")
    env["TELECLAUDE_SYSTEM_ROLE"] = "integrator"
...
@pytest.mark.integration
@pytest.mark.timeout(5)
def test_feature_branch_push_remains_allowed_during_cutover(tmp_path: Path) -> None:
    canonical_root = tmp_path / "repo"
    ...
    env["FAKE_GIT_BRANCH"] = "feature/cutover"
    env["FAKE_GIT_DIR"] = str(canonical_root / ".git" / "worktrees" / "slug")
    env["TELECLAUDE_SYSTEM_ROLE"] = "worker"

    result = subprocess.run(
        [str(wrapper), "push", "origin", "feature/cutover"],
        cwd=worktree,
        env=env,
        capture_output=True,
        text=True,
    ...
    assert result.returncode == 0
    assert "REAL_GIT_CALLED push origin feature/cutover" in result.stdout
