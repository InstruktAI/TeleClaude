...
- _Member session scoping:_ Member and contributor sessions use scoped tool filtering — deployment, session termination, agent status changes, and escalation are excluded. Members get project-level access without system-level control.
- _Visibility-based context filtering:_ Documentation snippets are tagged with visibility levels. Customers and members see only public knowledge; internal architecture, policies, and procedures are reserved for admins. The context window itself is role-aware.
...
We take this seriously and we're transparent about the gap between "gated" and "hardened." The three-tier access model (customer, member, admin) is implemented and enforced. Fine-tuning continues — tightening individual tool scopes and restricting system binary access for customer sessions — but the foundational security gates are operational.
