...
2. **Title only updates ONCE** - After the first `agent_stop` event sets the title, subsequent stop events skip title updates entirely due to a "New session" suffix check.
...
- No "New session" check - always update
...
1. **Remove "New session" check** in `_update_session_title()`:
   - Delete the regex guard: `if not re.search(r"New session( \(\d+\))?$", session.title): return`
