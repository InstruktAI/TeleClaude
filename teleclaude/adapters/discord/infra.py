...
class InfrastructureMixin(TeamChannelsMixin, ProvisioningMixin):
    ...
    async def _validate_channel_id(self, channel_id: int | None) -> int | None:
        ...
        if channel_id is None:
            return None
        ch = await self._get_channel(channel_id)  # type: ignore[attr-defined]
        if ch is None:
    ...
    async def _ensure_discord_infrastructure(self) -> None:
        ...
        self._announcements_channel_id = await self._validate_channel_id(self._announcements_channel_id)  # type: ignore[attr-defined]
        if self._announcements_channel_id is None:  # type: ignore[attr-defined]
            ch_id = await self._find_or_create_text_channel(guild, ops_cat, "announcements")
            if ch_id is not None:
                self._announcements_channel_id = ch_id  # type: ignore[attr-defined]
                changes["announcements_channel_id"] = ch_id

        self._general_channel_id = await self._validate_channel_id(self._general_channel_id)  # type: ignore[attr-defined]
        if self._general_channel_id is None:  # type: ignore[attr-defined]
            ch_id = await self._find_or_create_text_channel(guild, ops_cat, "general")
            if ch_id is not None:
                self._general_channel_id = ch_id  # type: ignore[attr-defined]
                changes["general_channel_id"] = ch_id

        # --- Category: Help Desk ---
        hd_cat = await self._ensure_category(guild, "Help Desk", existing_cats, cat_changes)

        self._help_desk_channel_id = await self._validate_channel_id(self._help_desk_channel_id)  # type: ignore[attr-defined]
        if self._help_desk_channel_id is None:  # type: ignore[attr-defined]
            forum_id = await self._find_or_create_forum(guild, hd_cat, "Customer Sessions", "Customer support sessions")
            if forum_id is not None:
                self._help_desk_channel_id = forum_id  # type: ignore[attr-defined]
                changes["help_desk_channel_id"] = forum_id

        self._escalation_channel_id = await self._validate_channel_id(self._escalation_channel_id)  # type: ignore[attr-defined]
        if self._escalation_channel_id is None:  # type: ignore[attr-defined]
            forum_id = await self._find_or_create_forum(
                guild, hd_cat, "Escalations", "Human-admin escalation relay threads"
            )
            if forum_id is not None:
                self._escalation_channel_id = forum_id  # type: ignore[attr-defined]
                changes["escalation_channel_id"] = forum_id

        self._operator_chat_channel_id = await self._validate_channel_id(self._operator_chat_channel_id)  # type: ignore[attr-defined]
        if self._operator_chat_channel_id is None:  # type: ignore[attr-defined]
            ch_id = await self._find_or_create_text_channel(guild, hd_cat, "operator-chat")
            if ch_id is not None:
                self._operator_chat_channel_id = ch_id  # type: ignore[attr-defined]
        ...
        self._all_sessions_channel_id = await self._validate_channel_id(self._all_sessions_channel_id)  # type: ignore[attr-defined]
        if self._all_sessions_channel_id is None:  # type: ignore[attr-defined]
            forum_id = await self._find_or_create_forum(
                guild, proj_cat, "Unknown", "Sessions from unrecognized project paths"
            )
            if forum_id is not None:
                self._all_sessions_channel_id = forum_id  # type: ignore[attr-defined]
    ...
    async def _ensure_category(
        ...
        *,
        ...
        if cached_id is not None:
            cat = await self._get_channel(cached_id)  # type: ignore[attr-defined]
            if cat is not None:
                return cat
        ...
        if cat is not None:
            cat_id = self._parse_optional_int(getattr(cat, "id", None))  # type: ignore[attr-defined]
        ...
        cat_id = self._parse_optional_int(getattr(cat, "id", None)) if cat else None  # type: ignore[attr-defined]
        if cat_id is not None:
            cat_changes[key] = cat_id
        return cat
    ...
    async def _find_category_by_name_robust(self, guild: object, name: str) -> tuple[object | None, bool]:
        ...
        try:
            fetched = await self._require_async_callable(fetch_fn, label="guild.fetch_channels")()  # type: ignore[attr-defined]
    ...
    def _resolve_parent_forum_id(self, channel: object | None) -> int | None:
        ...
        return self._parse_optional_int(getattr(parent, "id", None))  # type: ignore[attr-defined]
    ...
    def _build_session_launcher_view(self) -> object:
        ...
        return SessionLauncherView(enabled_agents=self._get_enabled_agents(), on_launch=self._handle_launcher_click)  # type: ignore[attr-defined]

    async def _post_or_update_launcher(self, forum_id: int) -> None:
        if self._client is None or not self._multi_agent:  # type: ignore[attr-defined]
            return

        forum_channel = await self._get_channel(forum_id)  # type: ignore[attr-defined]
        if forum_channel is None:
            logger.warning("Cannot post launcher for forum %s: channel not found", forum_id)
            return

        if not self._is_forum_channel(forum_channel):  # type: ignore[attr-defined]
        ...
        if (
            ...
            launcher_thread = await self._get_channel(int(existing_thread_id))  # type: ignore[attr-defined]
        ...
        created = await self._require_async_callable(create_thread_fn, label="Discord forum create_thread")(  # type: ignore[attr-defined]
            name=launcher_title,
        ...
        launcher_thread_id = self._parse_optional_int(getattr(launcher_thread, "id", None))  # type: ignore[attr-defined]
        ...
        launcher_message_id = self._parse_optional_int(launcher_message_id_raw)  # type: ignore[attr-defined]
    ...
    async def _resolve_interaction_forum_id(self, interaction: object) -> int | None:
        ...
        interaction_channel_id = self._parse_optional_int(getattr(interaction, "channel_id", None))  # type: ignore[attr-defined]
        if interaction_channel_id is None:
            return None

        if channel is not None and self._is_forum_channel(channel):  # type: ignore[attr-defined]
            return interaction_channel_id

        resolved_channel = await self._get_channel(interaction_channel_id)  # type: ignore[attr-defined]
        ...
        if resolved_channel is not None and self._is_forum_channel(resolved_channel):  # type: ignore[attr-defined]
            return interaction_channel_id
        return interaction_channel_id
    ...
    async def _pin_launcher_message(self, message: object, *, forum_id: int) -> None:
        ...
        try:
            await self._require_async_callable(pin_fn, label="Discord message pin")()  # type: ignore[attr-defined]
        except Exception as exc:
            logger.warning(
    ...
    async def _pin_launcher_thread(self, thread: object, *, forum_id: int) -> None:
        ...
        try:
            await self._require_async_callable(edit_fn, label="Discord thread edit (pin)")(pinned=True)  # type: ignore[attr-defined]
        except Exception as exc:
            logger.warning(
    ...
    async def _sync_project_forum_positions(self, category: object | None) -> None:
        ...
        category_id = self._parse_optional_int(getattr(category, "id", None))  # type: ignore[attr-defined]
        ...
        for td in trusted_dirs:
            if td.discord_forum is None:
                continue
            channel = await self._get_channel(td.discord_forum)  # type: ignore[attr-defined]
            if channel is None:
                continue
            ...
            ch_cat_id = self._parse_optional_int(getattr(channel, "category_id", None))  # type: ignore[attr-defined]
            ...
            if needs_move or needs_reorder:
                try:
                    ...
                    await self._require_async_callable(edit_fn, label="forum sync")(**kwargs)  # type: ignore[attr-defined]
                    if needs_move:
