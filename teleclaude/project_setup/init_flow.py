...
def _should_bootstrap_help_desk(project_root: Path) -> bool:
    """Check if help desk bootstrap should run for this project."""
    try:
        from teleclaude.config import config

        return bool(config.computer.help_desk_dir)
    except Exception:
        logger.debug("Could not check help desk config; skipping bootstrap")
        return False
