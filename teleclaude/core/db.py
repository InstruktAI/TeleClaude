...
from teleclaude.constants import DB_IN_MEMORY, HUMAN_ROLE_ADMIN, HUMAN_ROLE_CUSTOMER
...
def resolve_session_principal(session: "Session") -> tuple[str, str]:
    ...
    - Human session (human_email present): principal="human:<email>", role=human_role
    - Anonymous human session (human_role but no email): principal="session:<id>", role=human_role
    - System/job session (no human_role): principal="system:<id>", role="customer"

    The default role is always "customer" (least privilege). Only sessions with
    an explicitly resolved human_role get elevated access.
    """
    role = session.human_role or HUMAN_ROLE_CUSTOMER
    if session.principal:
        return session.principal, role
    if session.human_email:
        return f"human:{session.human_email}", role
    if session.human_role:
        return f"session:{session.session_id}", role
    # System/job session — least privilege by default.
    return f"system:{session.session_id}", HUMAN_ROLE_CUSTOMER
