"""Reusable history search functions for agent session transcripts.

These functions were extracted from scripts/history.py so both the standalone
script and the telec CLI can share the same logic.
"""
...
def display_combined_history(agents: List[AgentName], search_term: str = "", limit: int = 20) -> None:
    ...
    for i, entry in enumerate(all_results, 1):
        row = (
            f"{i:>4} | {entry['timestamp']:<17} | {entry['agent']:<8}"
            f" | {entry['project']:<20} | {entry['topic']:<70} | {entry['session_id']:<12}"
        )
        print(row)
