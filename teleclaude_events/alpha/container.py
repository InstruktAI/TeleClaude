"""Alpha container lifecycle manager — Docker sidecar for experimental cartridges.

Manages start/stop/health-check of the teleclaude-alpha-runner Docker container.
...
from teleclaude_events.alpha.protocol import AlphaRequest, AlphaResponse, read_frame, request_to_dict, write_frame
...
class AlphaContainerManager:
    def __init__(
        self,
        socket_path: str,
        cartridges_dir: str,
        image: str = "teleclaude-alpha-runner",
    ...
    async def start(self) -> None:
        ...
        if not self._check_docker_available():
            logger.warning("Docker not available — alpha container disabled")
            self._docker_unavailable = True
            await self._emit_event("system.alpha-container.docker-unavailable", "Docker not available or not running")
            return
        ...
        cmd = [
            ...
            "-v",
            f"{cartridges_dir}:/alpha-cartridges:ro",
            "-v",
            f"{socket_dir}:/run/alpha:rw",
            "-e",
            f"ALPHA_SOCKET_PATH=/run/alpha/{Path(self._socket_path).name}",
            "-e",
        ...
        try:
            ...
            logger.info("Alpha container started: %s", self._container_id[:12])
        except (subprocess.TimeoutExpired, OSError) as exc:
            raise RuntimeError(f"Failed to start alpha container: {exc}") from exc

        # Wait for socket to appear
        deadline = asyncio.get_event_loop().time() + _SOCKET_WAIT_TIMEOUT
        while asyncio.get_event_loop().time() < deadline:
            if Path(self._socket_path).exists():
                logger.info("Alpha runner socket ready")
                return
            await asyncio.sleep(_SOCKET_POLL_INTERVAL)

        # Timed out waiting — stop container and fail
        await self.stop()
        raise RuntimeError(f"Timed out waiting for alpha socket at {self._socket_path}")
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            logger.info("Alpha container stopped: %s", container_id[:12])
        except (subprocess.TimeoutExpired, OSError, subprocess.SubprocessError) as exc:
            logger.warning("Failed to stop alpha container %s: %s", container_id[:12], exc)
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            request = AlphaRequest(cartridge_name="__ping__", envelope={}, catalog_snapshot=[])
            ...
            logger.debug("Alpha health check failed: %s", exc)
            return False

    async def restart(self) -> None:
        self._restart_count += 1
        logger.warning("Alpha container restart attempt %d/%d", self._restart_count, _MAX_RESTART_ATTEMPTS)

        await self.stop()

        if self._restart_count > _MAX_RESTART_ATTEMPTS:
            logger.error("Alpha container permanently failed after %d restarts", self._restart_count - 1)
            self._permanently_failed = True
            await self._emit_event(
                "system.alpha-container.unhealthy",
                f"Alpha container failed {self._restart_count - 1} consecutive health checks",
            )
            return
        ...
        try:
            ...
            logger.error("Alpha container restart failed: %s", exc)
            if self._restart_count >= _MAX_RESTART_ATTEMPTS:
                self._permanently_failed = True
                await self._emit_event(
                    "system.alpha-container.unhealthy",
                    f"Alpha container restart failed after {_MAX_RESTART_ATTEMPTS} attempts: {exc}",
                )
    ...
    async def watch_health(self, shutdown_event: asyncio.Event) -> None:
        while not shutdown_event.is_set():
            ...
            if self.is_running and not await self.health_check():
                logger.warning("Alpha container health check failed — restarting")
                await self.restart()
    ...
    async def watch_cartridges_dir(self, shutdown_event: asyncio.Event) -> None:
        while not shutdown_event.is_set():
            ...
            if self._has_cartridges and not self.is_running and not self._permanently_failed and not self._docker_unavailable:
                logger.info("Alpha cartridges detected — starting container")
                try:
                    await self.start()
                except Exception as exc:
                    logger.error("Failed to start alpha container: %s", exc)

            elif not self._has_cartridges and had_cartridges and self.is_running:
                logger.info("Alpha cartridges removed — stopping container")
                await self.stop()

    async def _emit_event(self, event_type: str, description: str) -> None:
        if self._producer is None:
            logger.warning("Alpha event not emitted (no producer): %s — %s", event_type, description)
            return
        ...
        try:
            await self._producer.emit(
                EventEnvelope(
                    event=event_type,
                    source="alpha-container-manager",
            ...
            logger.warning("Failed to emit alpha event %s: %s", event_type, exc)
