#!/usr/bin/env bash
# Demo render script for themed-primary-color delivery
...
if [[ "$CURRENT_MAJOR" != "$SNAPSHOT_MAJOR" ]]; then
    echo "Warning: snapshot is $SNAPSHOT_VERSION, current is $CURRENT_VERSION"
...
echo ""
echo "Quality: No critical findings. $(jq -r '.metrics.important_findings' "$SNAPSHOT") important finding, $(jq -r '.metrics.suggestions' "$SNAPSHOT") suggestions."
echo ""
