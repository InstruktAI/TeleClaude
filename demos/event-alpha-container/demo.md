...
```bash
# Verify container security flags (requires running container)
docker inspect $(docker ps -qf name=teleclaude-alpha-runner) 2>/dev/null | python3 -c "
import json, sys
...
make test -- tests/unit/test_alpha/ -v 2>&1 | tail -20
```
