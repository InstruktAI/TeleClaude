...
- [x] ~~Any task that changes behavior starts with the named failing test(s) before
      production code.~~ N/A — test suite removed; will be recreated separately.
- [x] ~~Use targeted tests while iterating.~~ N/A — test suite removed.
- [x] Reuse existing TeleClaude surfaces. Do not introduce a parallel project-catalog
...
- [x] Write a procedure snippet that guides the AI during project analysis.
- [x] Define per-language/framework analysis checklists (Python, TypeScript/Node,
...
- [x] Define convention inference rules from git history, file structure, and test
      patterns.
- [x] Define sampling strategy for large codebases (file count thresholds, directory
      prioritization, fallbacks when the context window is tight).
- [x] Define the decision boundary: when to infer, when to preserve ambiguity, and
      when to leave a placeholder for human follow-up.
- [x] Include snippet-output guidance for the generated project snippets and the
      project-local `AGENTS.md` bootstrap content.
- [x] Include valid frontmatter with `id`, `description`, `type: procedure`, and
      `scope`.
...
- [x] Define the discovery-to-taxonomy mapping: which analysis dimensions map to
...
- [x] Define naming conventions for generated snippet IDs under `project/`.
- [x] Define the expected file placement per taxonomy type under `docs/project/`.
- [x] Define the frontmatter template for generated snippets, including
      `generated_by: telec-init` and `generated_at: <ISO8601>`.
- [x] Define merge rules for re-analysis: update auto-generated sections, preserve
      human-authored sections, and keep the operation idempotent.
- [x] Define how generated project-local agent bootstrap source content coexists
...
- [x] Start with failing tests for:
...
- [x] After the existing plumbing steps complete, detect whether enrichment should run.
- [x] On first init, prompt to run enrichment or skip.
- [x] On re-init, offer refresh or skip.
- [x] Prompt for release-channel enrollment during init using the existing
      `deployment.channel` / `deployment.pinned_minor` fields and show the
      current enrollment on rerun.
- [x] Validate that stable-channel enrollment collects a pinned minor and persist
      the chosen values to `teleclaude.yml` without introducing new config keys
      or YAML sections.
- [x] Launch `telec sessions run --command /telec-init-analyze --project <root>`
      and print a status message with enough session context to follow the run.
- [x] Handle a declined enrichment path gracefully and continue with normal init
      completion output.
- [x] Update `telec init` help/usage text so the enrichment option is visible in the
      user-facing command description.
- [x] Update the user-facing command docs that mirror `telec init` behavior
...
- [x] Start with failing tests for snippet writing, directory creation, existing
      snippet detection, merge preservation, metadata persistence, and rejection of
      unknown or unsafe snippet IDs.
- [x] Implement `write_snippet(project_root, snippet_id, content, metadata)` so it
      writes schema-valid snippets under `docs/project/`.
- [x] Implement `read_existing_snippets(project_root)` so re-analysis can inspect
      current generated snippets and metadata markers.
- [x] Implement `merge_snippet(existing, generated)` so human-authored sections
      survive refresh runs.
- [x] Implement `ensure_taxonomy_directories(project_root, snippet_ids)` for the
      taxonomy directories implied by the validated output, with `design/`,
      `policy/`, and `spec/` covered by the baseline fixtures.
- [x] Validate analysis output against schema-valid project snippet IDs and
...
- [x] Write and read `.telec-init-meta.yaml` with timestamps, counts, generated
...
- [x] Define the slash command invoked by `/telec-init-analyze`.
- [x] Load the guidance and scaffolding snippets from Tasks 1.1 and 1.2 via
      explicit required reads.
- [x] Instruct the session to inspect the repo with the guidance checklist and emit
      structured output that the enrichment writer can persist.
- [x] Instruct the session to generate initial project-specific agent bootstrap
...
- [x] Instruct the session to leave a concise created/updated-files summary, run
      `telec sync --validate-only`, and finish cleanly.
- [x] Make the required-read references observable in the transcript so tests can
...
- [x] Add or update a targeted regression test that proves snippets under
      `docs/project/` are still discovered by `iter_snippet_roots()`.
- [x] Verify `telec docs index` lists the generated snippets after enrichment.
- [x] Verify `telec docs get <generated-snippet-id>` returns non-empty,
...
- [x] Start with failing tests proving a first-time `telec init` on a repo without a
...
- [x] If manifest registration currently depends on `teleclaude.yml` existing before
      sync begins, adjust the init/sync ordering so first-run projects are registered
      after config creation.
- [x] Reuse the existing project catalog behavior instead of introducing a parallel
      registration file or init-only manifest path.
- [x] Surface release-channel enrollment during init and persist it only through
      `deployment.channel` and `deployment.pinned_minor`, avoiding new config
      keys or YAML sections.
- [x] For `stable`, collect and validate `deployment.pinned_minor`; for `alpha`
...
- [x] Start with failing tests for re-init refresh/skip detection, changed-file-only
      rewrites, and preservation of human-authored sections.
- [x] Detect existing auto-generated snippets via `generated_by` metadata.
- [x] Compare existing snippet content with the new analysis output.
- [x] Apply the merge rules from Task 1.2 and log what was updated versus preserved.
- [x] Rewrite only generated files whose merged output actually changed on refresh runs.
...
- [x] Start with a failing test that asserts `.telec-init-meta.yaml` is written with
...
- [x] Persist:
      ...
      ```
- [x] Read the metadata file during re-analysis to inform merge decisions and logs.
...
- [x] Keep the existing init plumbing regression coverage green so hooks, sync, and
...
- [x] Run only the targeted pytest files for the touched init/enrichment/command/index
      paths while developing.
- [x] Run `telec todo demo validate telec-init-enrichment`.
- [x] Run the normal pre-commit hook path before commit; if hooks or targeted checks
      expose wider issues, escalate to broader suites.
- [x] Manual test: run `telec init` on a sample project, accept enrichment, verify
      snippet quality, `telec docs get`, `telec projects list`, release-channel
      persistence in `teleclaude.yml`, and re-init preservation. (Deferred to post-merge demo.)
...
- [x] Update `todos/telec-init-enrichment/demo.md` so the executable steps use the
      taxonomy paths under `docs/project/design/`, `docs/project/policy/`, and
      `docs/project/spec/`.
- [x] Ensure the demo covers `telec docs get`, `telec projects list`, and re-init
      preservation in observable commands.
- [x] Confirm requirements are reflected in code changes with no silent scope drops.
- [x] Confirm implementation-plan task checkboxes are all marked `[x]`.
- [x] Document any deferrals explicitly in `deferrals.md` (if applicable).
- [x] Deferred: event emission during init (depends on `event-envelope-schema`).
- [x] Deferred: mesh registration during init (depends on `mesh-architecture`).
