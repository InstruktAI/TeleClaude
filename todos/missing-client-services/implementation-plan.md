...
- [x] Create `send_discord_dm(user_id: str, content: str, *, token_env: str = "DISCORD_BOT_TOKEN", timeout_s: float = 10.0) -> str`
- [x] Step 1: Read bot token from `os.getenv(token_env)`, raise `ValueError` if missing
- [x] Step 2: Validate content is not empty, raise `ValueError` if so
- [x] Step 3: Create DM channel via `POST https://discord.com/api/v10/users/@me/channels` with `{"recipient_id": user_id}` and `Authorization: Bot {token}` header
- [x] Step 4: Extract `channel_id` from response
- [x] Step 5: Send message via `POST https://discord.com/api/v10/channels/{channel_id}/messages` with `{"content": content[:2000]}` (Discord's 2000-char limit)
- [x] Step 6: Log warning if content was truncated
- [x] Step 7: Return message ID as string
- [x] Step 8: Handle HTTP errors (status >= 400) with clear `RuntimeError` messages, matching telegram.py's error handling pattern

### Task 1.2: Create `teleclaude/services/whatsapp.py`

**File(s):** `teleclaude/services/whatsapp.py`

- [x] Create `send_whatsapp_message(phone_number: str, content: str, *, phone_number_id: str, access_token: str, api_version: str = "v21.0", timeout_s: float = 10.0) -> str`
- [x] Step 1: Validate phone_number, content, phone_number_id, access_token are non-empty
- [x] Step 2: Build URL: `https://graph.facebook.com/{api_version}/{phone_number_id}/messages`
- [x] Step 3: Build payload: `{"messaging_product": "whatsapp", "to": phone_number, "type": "text", "text": {"body": content[:4096]}}` (WhatsApp's limit)
- [x] Step 4: POST with `Authorization: Bearer {access_token}` header
- [x] Step 5: Extract `messages[0].id` from response
- [x] Step 6: Log warning if content was truncated
- [x] Step 7: Return message ID as string
- [x] Step 8: Handle HTTP errors with clear `RuntimeError` messages
...
- [x] Create `DiscordDeliveryAdapter` class following `TelegramDeliveryAdapter` pattern exactly
- [x] Constructor: `__init__(self, user_id: str, send_fn: Callable[..., Coroutine], min_level: int = int(EventLevel.WORKFLOW))`
- [x] Method: `on_notification(self, notification_id, event_type, level, was_created, is_meaningful)` — same signature as Telegram adapter
- [x] Filter: skip if `not was_created` or `level < self._min_level`
- [x] Format message: `f"[{event_type}] Notification #{notification_id} created."`
- [x] Call `send_fn(user_id=self._user_id, content=message)`
- [x] Catch all exceptions, log and continue (never crash the pipeline)

### Task 2.2: Create `teleclaude_events/delivery/whatsapp.py`

**File(s):** `teleclaude_events/delivery/whatsapp.py`

- [x] Create `WhatsAppDeliveryAdapter` class following same pattern
- [x] Constructor: `__init__(self, phone_number: str, send_fn: Callable[..., Coroutine], min_level: int = int(EventLevel.WORKFLOW))`
- [x] Same `on_notification` filtering and message formatting
- [x] Call `send_fn(phone_number=self._phone_number, content=message)`
...
- [x] Add imports for `DiscordDeliveryAdapter` and `WhatsAppDeliveryAdapter`
- [x] Update `__all__` to include both
...
- [x] After the existing Telegram delivery adapter registration block (lines 1693-1718), add a Discord block:
...
- [x] Discord service tests (matching `test_telegram.py` pattern):
...
- [x] Discord delivery adapter tests (matching `test_telegram_adapter.py` exactly):
...
- [x] WhatsApp delivery adapter tests — same five test cases

### Task 4.3: Quality checks

- [x] Run `make test`
- [x] Run `make lint` — pre-existing failure in `teleclaude/cli/tui/animations/general.py` (loose dict typings); not introduced by this change
