# person-knowledge-level — Input

## Person Knowledge Level — Feature

Add a `knowledge` field to PersonEntry so agents know the human's knowledge level at session start and derive all behavioral calibration from it.

### What

One field on the person: `knowledge: novice | intermediate | advanced | expert`

- **novice** — little to no technical knowledge
...
Agents currently have no way to know who they're talking to. They ask questions that the human can't answer (novice) or doesn't want to answer (expert). Communication is at the wrong level. The knowledge level is the single static fact from which agents derive all behavioral calibration — communication register, autonomy level, what to surface vs handle silently.
...
3. Extend it to look up the matching PersonEntry from global config to get `knowledge`
...
1. **`teleclaude/config/schema.py`** — add `knowledge: Literal["novice", "intermediate", "advanced", "expert"] = "intermediate"` to `PersonEntry`
2. **`teleclaude/hooks/receiver.py`** — extend `_print_memory_injection()` to look up person entry by human_email from global config, prepend knowledge line to context
3. **`teleclaude/cli/config_cli.py`** — add `--knowledge` to `_people_add()` and `_people_edit()`, add to `PersonInfo` dataclass
4. **`teleclaude/api_models.py`** — add `knowledge` to `PersonDTO`
