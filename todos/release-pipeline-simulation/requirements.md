...
3.  **Safety:** We need to prove the Arbiter correctly handles conservative overrides and three-way disagreement tiebreakers _before_ we trust it with production releases.
...
- [ ] **Scenarios:** The pipeline must run the Arbiter against 3 pre-canned scenarios:
  1.  **Unanimous Patch:** All 3 agents say "patch". -> Result: `release_authorized=true`, `target_version="patch"`.
  2.  **Split Vote:** 2 Patch, 1 Minor. -> Result: `release_authorized=true`, `target_version="patch"` (majority wins).
  3.  **Conservative Override:** Majority "None", Minority "Patch" with `contract_changes` present. -> Result: `release_authorized=true`, `target_version="patch"`, rationale mentions "contract changes" and "overriding" (minority trusted).
- [ ] **Decision Verification:** The test pipeline must assert the `arbiter-decision.json` fields match the expected outcome for each scenario (authorized, version, rationale substring).
