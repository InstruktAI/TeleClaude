...
- [x] Reduce `CallbackAction` enum to generic actions:
...
- [x] Remove the old per-agent enum values.
- [x] Update `AGENT_SELECT_ACTIONS` and `AGENT_START_ACTIONS` sets to use new enums.

### Task 1.2: Legacy payload fallback map

**File(s):** `teleclaude/adapters/telegram/callback_handlers.py`

- [x] Add `LEGACY_ACTION_MAP: dict[str, tuple[str, str]]` that maps old action prefixes to
  ...
  ```
- [x] In `_handle_callback_query`, after splitting `action_raw`, check `LEGACY_ACTION_MAP`
...
- [x] New canonical format uses two colons: `action:agent_name:arg`. Parse accordingly:
  split on `:` with maxsplit=2.
- [x] For `AGENT_SELECT` and `AGENT_RESUME_SELECT`: extract `agent_name` and pass to
  `_handle_agent_select`.
- [x] For `AGENT_START` and `AGENT_RESUME_START`: extract `agent_name` and `project_idx`,
  pass to `_handle_agent_start`.
- [x] Validate `agent_name` against `AgentName.from_str()`. If invalid, log and return.

### Task 1.4: Update `_handle_agent_select`

**File(s):** `teleclaude/adapters/telegram/callback_handlers.py`

- [x] Change signature to accept `agent_name: str` and `is_resume: bool` instead of raw
  `action: str`.
- [x] Remove `mode_map` dict. Derive `callback_prefix` and `mode_label` from agent_name
  and is_resume:
...
- [x] Change signature to accept `agent_name: str` and `is_resume: bool` instead of raw
  `action: str`.
- [x] Remove `event_map` dict. Derive `auto_command` directly:
  - `auto_command = f"agent_resume {agent_name}" if is_resume else f"agent {agent_name}"`
- [x] Validate agent_name with `AgentName.from_str()` before proceeding.

### Task 1.6: Dynamic heartbeat keyboard

**File(s):** `teleclaude/adapters/telegram_adapter.py`

- [x] Replace hardcoded agent rows in `_build_heartbeat_keyboard` with a loop over
  `get_enabled_agents()`.
- [x] For each enabled agent, add a row with two buttons:
  - `"🤖 New {agent.title()}"` → `callback_data=f"asel:{agent}:{bot_username}"`
  - `"🔄 Resume {agent.title()}"` → `callback_data=f"arsel:{agent}:{bot_username}"`
- [x] Keep first row (Tmux Session) unchanged.
- [x] Agent-specific emoji mapping (optional, keep generic 🤖 if simplicity is preferred).
...
- [x] Update `tests/unit/test_telegram_menus.py`:
...
- [x] Test agent start with new payloads creates correct `auto_command`.
- [x] Run `make test`
