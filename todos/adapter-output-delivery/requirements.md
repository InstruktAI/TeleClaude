...
Guarantee complete, continuous output delivery from agent sessions to all UI adapters, and ensure user input from any adapter is reflected to all others with correct provenance.
...
2. **User input reflection across adapters** — when a user sends input from any adapter (terminal, Telegram, Discord, MCP), that input is broadcast to all OTHER UI adapters as a formatted message (`"{SOURCE} @ {computer_name}:\n\n{text}"`).
3. **Session ownership lineage** — human role/email lineage is propagated from parent sessions; when no human lineage exists, ownership defaults to system.
4. **Origin semantics** — `last_input_origin` is per-message provenance only and must not be inherited from parent sessions.
...
- [ ] MCP-origin input is broadcast like any other input, with actor identity resolved from lineage (or system fallback).
- [ ] Child sessions inherit `human_email`/`human_role` from parent lineage when not explicitly provided.
- [ ] Child sessions do not inherit parent `last_input_origin`; each session records its own creation/input origin.
