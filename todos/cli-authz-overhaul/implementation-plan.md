...
- [x] Add `CommandAuth` dataclass adjacent to `CommandDef`:
  ```python
  ...
  ```
- [x] Add optional `auth: CommandAuth | None = None` field to `CommandDef`

### Task 1.2: Define role constants for auth

**File(s):** `teleclaude/cli/telec.py`

- [x] Import `ROLE_WORKER`, `ROLE_ORCHESTRATOR`, and human role constants from `teleclaude.constants`
- [x] Define shorthand sets for common role patterns to keep `CLI_SURFACE` readable:
  ```python
  ...
  ```
- [x] Keep these as module-level constants in `telec.py`, near `CLI_SURFACE`

### Task 1.3: Implement `is_command_allowed()`

**File(s):** `teleclaude/cli/telec.py`

- [x] Add function that resolves a command path to its `CommandAuth` and checks the two-axis rule:
  ```
  allowed = (system_role in auth.system) AND (human_role == "admin" OR human_role in auth.human)
  ```
- [x] Special case: `sessions escalate` — admin is explicitly excluded. Implement via an
...
- [x] Handle `None` system_role (non-session callers like TUI/terminal): treat as orchestrator
  for system-role check (non-session callers are never workers).
- [x] Handle `None` human_role: deny all human-role-gated commands (fail closed).
- [x] Return `False` for unknown command paths (fail closed).
...
- [x] Add `auth=CommandAuth(...)` to every leaf `CommandDef` in `CLI_SURFACE`, following
...
- [x] The `todo demo` command takes a positional arg (`list|validate|run|create`) rather
...
- [x] These currently use `HELP_SUBCOMMAND_EXPANSIONS` for display but don't have explicit
...
- [x] Update `sessions end` row: add member ✅ (self-scoped at API layer)
- [x] Update `sessions restart` row: remove member, keep admin only
- [x] Update `sessions revive` row: remove member, keep admin only
- [x] Verify `sessions escalate` row already shows admin ❌, worker ✅
- [x] Verify `roadmap list` row already shows worker ❌
- [x] Add note about self-scoping for `sessions end` (member can only end own sessions)
...
- [x] Test admin bypasses human-role check for all commands
- [x] Test admin is DENIED `sessions escalate`
- [x] Test worker is denied orchestrator-only commands
- [x] Test worker is allowed worker-permitted commands (result, file, widget, escalate, docs, etc.)
- [x] Test member permissions match the matrix
- [x] Test contributor restrictions (no session start/send/run)
- [x] Test newcomer restrictions (read-heavy)
- [x] Test customer restrictions (only version, docs, auth, escalate)
- [x] Test unknown command path returns False
- [x] Test None system_role treated as orchestrator
- [x] Test None human_role denied for all human-gated commands

### Task 4.2: Completeness test — every leaf has auth

**File(s):** `tests/unit/test_command_auth.py` (new)

- [x] Walk `CLI_SURFACE` recursively, collect all leaf `CommandDef` nodes
- [x] Assert every leaf has `auth is not None`
- [x] This test prevents future regressions when new commands are added without auth
...
- [x] Document follow-on todos needed (WS2-WS11) in deferrals if applicable
