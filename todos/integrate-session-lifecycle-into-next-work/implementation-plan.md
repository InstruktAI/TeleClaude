...
- [x] Add `verify_artifacts(worktree_cwd: str, slug: str, phase: str) -> tuple[bool, str]` function
- [x] Build phase checks:
...
- [x] Return `(passed: bool, report: str)` where report lists each check with pass/fail

### Task 1.2: Add CLI command `telec todo verify-artifacts`

**File(s):** `teleclaude/cli/telec.py`, `teleclaude/cli/tool_commands.py`

- [x] Register `verify-artifacts` as a subcommand under `todo` in `CLI_SURFACE`
- [x] Accept arguments: `<slug>` (required), `--phase <build|review>` (required), `--cwd` (optional, defaults to project root)
- [x] Call `verify_artifacts()` and print the report
- [x] Exit 0 on pass, exit 1 on failure

### Task 1.3: Integrate verification into state machine

**File(s):** `teleclaude/core/next_machine/core.py`

- [x] After `run_build_gates()` passes in `next_work()`, also call `verify_artifacts(worktree_cwd, slug, "build")` — both must pass before dispatching review
- [x] In `POST_COMPLETION["next-review"]`: add instruction for orchestrator to run `telec todo verify-artifacts <slug> --phase review` before processing the verdict
- [x] If verification fails, format an error similar to `format_build_gate_failure()` that tells the orchestrator what's missing
...
- [x] Split `POST_COMPLETION["next-review"]` into two paths:
...
- [x] In the `review_status == CHANGES_REQUESTED` branch of `next_work()`:
...
- [x] Add guidance that when a `--direct` peer link is established with a reviewer, the fixer should:
...
- [x] Add note that the fixer may receive direct messages from the reviewer during work — this is expected and the fixer should respond to review feedback inline
...
- [x] Add `@~/.teleclaude/docs/general/principle/session-lifecycle.md` to the `## Required reads` section
- [x] Run `telec sync` to regenerate artifacts

### Task 3.2: Strengthen POST_COMPLETION lifecycle discipline

**File(s):** `teleclaude/core/next_machine/core.py`

- [x] Add artifact verification step to all POST_COMPLETION entries before ending sessions
- [x] Add explicit "verify artifact delivery before ending session" instruction to each POST_COMPLETION block
- [x] Ensure every POST_COMPLETION path ends with `telec sessions end <session_id>` as the last action before calling next_work (already true for most, verify consistency)
- [x] Add signal session concept: if a worker reports a non-recoverable error, POST_COMPLETION should instruct the orchestrator to keep the session alive as a signal for human attention instead of ending it
...
- [x] Test build phase verification: pass case (all tasks checked, commits exist, quality checklist populated)
- [x] Test build phase verification: fail case (unchecked tasks, no commits, empty checklist)
- [x] Test review phase verification: pass case (findings exist, verdict present, checklist populated)
- [x] Test review phase verification: fail case (template-only findings, missing verdict)
- [x] Test general checks: malformed state.yaml, inconsistent phase status
- [x] Test edge cases: missing files, empty files, partially filled templates
