...
- [ ] Add `_reconcile()` that calls `list-panes -F "#{pane_id}"` once, builds a set of live pane IDs, and removes entries from `session_to_pane` where the pane is dead
- [ ] Also prune `sticky_pane_ids`, `sticky_session_to_pane`, `session_pane_ids` in sync
- [ ] Clear `parent_pane_id` / `parent_session` / `parent_spec_id` if the parent pane is dead
- [ ] Call `_reconcile()` at the top of `apply_layout()` before any signature checks
