...
- [x] Add function `resolve_parser_agent(active_agent: str | None) -> AgentName`
- [x] If `active_agent` is `None` or empty string: log at `debug` level, return `AgentName.CLAUDE`
- [x] If `AgentName.from_str()` succeeds: return the resolved value (no log)
- [x] If `AgentName.from_str()` raises `ValueError`: log at `warning` level with the
      original value, return `AgentName.CLAUDE`
- [x] Import `logging` and use module logger (`logger = logging.getLogger(__name__)`)

### Task 1.2: Update `teleclaude/api/streaming.py` `_get_agent_name()`

**File(s):** `teleclaude/api/streaming.py`

- [x] Replace inline try/except in `_get_agent_name()` body with call to
      `resolve_parser_agent(session.active_agent)`
- [x] Remove the `or "claude"` default — the resolver handles `None`

### Task 1.3: Update `teleclaude/api_server.py` `/messages` endpoint

**File(s):** `teleclaude/api_server.py`

- [x] Replace inline try/except block (lines ~1096-1100) with call to
      `resolve_parser_agent(session.active_agent)`
- [x] Import `resolve_parser_agent` from `teleclaude.core.agents`

### Task 1.4: Update `teleclaude/utils/transcript.py` `_get_entries_for_agent()`

**File(s):** `teleclaude/utils/transcript.py`

- [x] The function already receives `AgentName` (enum), so no fallback change needed here.
...
- [x] No code change needed; this task is verification only.

### Task 1.5: Verify `get_transcript_parser_info()` safety

**File(s):** `teleclaude/utils/transcript.py`

- [x] `get_transcript_parser_info()` takes `AgentName` enum, not a string. Since the enum
      is exhaustive, `KeyError` cannot happen if callers use `resolve_parser_agent()` first.
- [x] Verify all callers pass `AgentName` enum values (not raw strings).
- [x] No code change needed; this task is verification only.
...
- [x] Add test `test_resolve_parser_agent_canonical_values` — parametrized over
      `("claude", AgentName.CLAUDE)`, `("codex", AgentName.CODEX)`, `("gemini", AgentName.GEMINI)`
- [x] Add test `test_resolve_parser_agent_none_defaults_to_claude` — asserts
      `resolve_parser_agent(None) == AgentName.CLAUDE`
- [x] Add test `test_resolve_parser_agent_empty_defaults_to_claude` — asserts
      `resolve_parser_agent("") == AgentName.CLAUDE`
- [x] Add test `test_resolve_parser_agent_unknown_defaults_to_claude` — asserts
      `resolve_parser_agent("gpt4") == AgentName.CLAUDE`
- [x] Add test `test_resolve_parser_agent_case_insensitive` — asserts
      `resolve_parser_agent(" CODEX ") == AgentName.CODEX`
- [x] Add test `test_resolve_parser_agent_logs_warning_for_unknown` — captures log output,
      asserts warning contains original value
- [x] Add test `test_resolve_parser_agent_logs_debug_for_none` — captures log output,
