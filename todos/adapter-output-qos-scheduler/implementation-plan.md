...
- [x] Simulate `N=20` active emitting sessions.
- [x] Confirm sustained operation without runaway flood-control retries.
- [x] Confirm queue stabilization and bounded staleness.

### Task 5.3: Runtime validation

- [x] `make restart`
- [x] `make status`
- [x] `instrukt-ai-logs teleclaude --since 15m --grep "Output cadence summary|Rate limited|qos|scheduler"`
- [x] Validate final-message delivery correctness on session completion.
...
- [x] Roll out behind feature flag.
- [x] Start with conservative Telegram output budget and observe metrics.
- [x] Enable Discord coalesce-only mode only after no-regression verification.
