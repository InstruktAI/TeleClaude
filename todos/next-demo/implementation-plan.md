...
- [ ] Act 1 (Challenge): One paragraph, no jargon, user's perspective. Source: `requirements.md` Goal section.
- [ ] Act 2 (Build): The most interesting technical choice. Source: `implementation-plan.md` overview + git diff stat.
- [ ] Act 3 (Gauntlet): Quality earned, not rework endured. Source: `review-findings.md` — criticals found/fixed, rounds survived.
- [ ] Act 4 (Numbers): Metrics table, scannable at a glance. Source: git + state.json.
...
- [ ] `demo.sh` is generated by the command, not a static template
- [ ] Script reads `snapshot.json` from its own directory (`$(dirname "$0")/snapshot.json`)
- [ ] Extracts `version` field, compares major component against current `pyproject.toml` version
- [ ] If major mismatch: prints compatibility message and exits 0 (no error, just skipped)
- [ ] If compatible: renders the demo via curl to the daemon API (`render_widget` endpoint) or falls back to formatted terminal output (jq + printf)
- [ ] Script is `chmod +x`
