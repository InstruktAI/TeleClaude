...
- [x] Gemini correctly identifies changes to public surfaces (CLI, MCP, Events, Config) by comparing the codebase to `docs/manifests/`.
- [x] Gemini produces a structured report (JSON) using the same schema as Claude and Codex.
- [x] Integration with our custom CI environment where `gemini-cli` is installed via `npm`.
- [x] The workflow successfully authenticates using `GOOGLE_API_KEY`.
