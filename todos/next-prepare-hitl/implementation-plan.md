...
- [ ] Modify `next_prepare()` function signature (line 366)
  - Add `hitl: bool = True` parameter

- [ ] Remove automatic slug resolution for HITL=true mode (lines 381-382)
  - When `hitl=True` and no slug provided, skip `resolve_slug()` call
  - When `hitl=False`, keep existing `resolve_slug()` behavior

- [ ] Delete `is_in_progress` check block (lines 397-404)
  - This check doesn't belong in prepare phase

- [ ] Replace `next-roadmap` dispatch with `next-prepare` (lines 385-394)
  - Change `command="next-roadmap"` to `command="next-prepare"`
  - Set `args=""`

- [ ] Add HITL branching for "no slug" case
...
- [ ] Add HITL branching for "missing requirements.md" case (lines 407-421)
...
- [ ] Add HITL branching for "missing implementation-plan.md" case (lines 424-438)
...
- [ ] Extract `hitl` parameter in `call_tool()` method (around line 776)
  - Default to `True` if not provided

- [ ] Update `teleclaude__next_prepare()` method signature (around line 2132)
  - Add `hitl: bool = True` parameter

- [ ] Pass `hitl` parameter to `next_prepare()` function call (around line 2152)

## Group 3: Testing

- [ ] Run `make lint` to verify no lint errors
- [ ] Run `make test` to verify existing tests pass
- [ ] Add/update unit tests for `next_prepare()` in `tests/unit/`
