...
- [x] Add `proficiency: Literal["novice", "intermediate", "advanced", "expert"] = "intermediate"`
...
- [x] In `_print_memory_injection()`, after the existing identity resolution block
...
- [x] Store `person_name` and `person_Proficiency` from the matched `PersonEntry`.
- [x] After building `context` from `_get_memory_context()`, prepend the proficiency line
      if a person was found: `Human in the loop: {person_name} ({person_Proficiency})`.
- [x] Adjust the early return: proceed with injection even if memory context is empty
...
- [x] In `PersonInfo` dataclass, add `proficiency: str | None = None`.
- [x] In `_people_list()`, populate `info.proficiency` from `person.proficiency` (the
      `PersonEntry` field via `getattr` for safety, defaulting to `"intermediate"`).
- [x] In `_people_add()`, read `opts.get("proficiency")` and pass it to `PersonEntry()`
      constructor.
- [x] In `_people_edit()`, add `"proficiency"` to the check for editable global entry
...
- [x] Update the "no changes" error message to include `--proficiency` in the list of
      valid flags.
...
- [x] In `_render_people()`, after rendering role and email for each person, append
...
- [x] Add unit test for `PersonEntry` schema: valid values accepted, invalid rejected.
- [x] Add unit test for `_print_memory_injection` proficiency line output: mock session
      row with `human_email`, mock `config.people` with matching person, verify output
      contains `Human in the loop:` prefix.
- [x] Add unit test for CLI `_people_add` with `--proficiency expert`: verify `PersonEntry`
      is created with `proficiency="expert"`.
- [x] Add unit test for CLI `_people_edit` with `--proficiency novice`: verify field update.
- [x] Run `make test`
