# DOR Report (Gate): codex-render-baseline
...
- Verification is concrete and testable rather than aspirational:
  ...
  replay tests for R5, `telec todo demo validate codex-render-baseline` for the
...
- `todos/codex-render-runtime/input.md` explicitly depends on this baseline todo.
- The roadmap previously contained stale inverse sequencing
  (`codex-render-baseline` after `codex-render-semantic-runtime`), which would have
