...
### 1. Integrator tool access whitelist

```bash
python3 -c "
from teleclaude.constants import ROLE_INTEGRATOR
from teleclaude.core.tool_access import is_tool_allowed, INTEGRATOR_ALLOWED_TOOLS
# Integrator can drive integration
assert is_tool_allowed(ROLE_INTEGRATOR, 'telec todo integrate'), 'should allow integrate'
# Integrator cannot dispatch workers
assert not is_tool_allowed(ROLE_INTEGRATOR, 'telec sessions run'), 'should block dispatch'
# Integrator cannot use orchestrator commands
assert not is_tool_allowed(ROLE_INTEGRATOR, 'telec todo work'), 'should block work'
print(f'Integrator whitelist OK ({len(INTEGRATOR_ALLOWED_TOOLS)} tools)')
"
```
...
### 4. CLI CommandAuth includes integrator
...
Show that `ROLE_INTEGRATOR` exists as a constant, has its own tool whitelist (6 tools),
and is recognized by the auth derivation layer. The integrator can observe sessions,
report results, and drive integration — but cannot dispatch sessions or mark phases.
...
Dead sessions without metadata no longer cause false positives.
