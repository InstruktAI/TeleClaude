...
- [ ] Add `session_tokens` table to `schema.sql`:
  ...
  ```
- [ ] Write migration `031_add_session_tokens.py` that creates the table and indexes
...
- [ ] Add `SessionToken(SQLModel, table=True)` with fields matching the schema: `token`, `session_id`, `principal`, `role`, `issued_at`, `expires_at`, `revoked_at`
...
- [ ] `issue_session_token(session_id, principal, role) -> str` — generate UUID token, store in `session_tokens` with `issued_at=now`, `expires_at=now+24h`. Return the token string.
...
- [ ] Add `resolve_session_principal(session: Session) -> tuple[str, str]` returning `(principal, role)`:
...
- [ ] The role is carried with the principal so authorization checks never encounter a role-less credential
...
- [ ] Add `principal_role: str | None = None` field to `CallerIdentity` dataclass
...
- [ ] Update `is_command_allowed()` signature to accept `principal: str | None = None` and `principal_role: str | None = None`
- [ ] Change the `human_role is None` denial (line ~1019) to:
  ```python
  if human_role is None and principal is None:
      return False
  ```
- [ ] When `principal` is present and `human_role` is None, use `principal_role` for the human-role allowlist check instead of re-reading the ledger downstream.
- [ ] Update `_is_tool_denied()` in `auth.py` to pass `identity.principal` and `identity.principal_role` to `is_command_allowed()`
...
- [ ] In `POST /sessions/run` handler (~line 1416): when `identity.principal` is present, add it to `channel_metadata`:
  ```python
  if identity.principal:
      channel_metadata = channel_metadata or {}
      channel_metadata["principal"] = identity.principal
  ```
- [ ] In `command_handlers.py` `create_session()` (~line 328): inherit principal from parent session when resolving identity, same pattern as human_email/human_role inheritance
- [ ] Store principal on the session record (add `principal` column to sessions table, or store in `session_metadata`) so child sessions can inherit it
...
- [ ] In `_bootstrap_session_resources()`, when resolving the principal: if the session has an inherited principal (from parent), use that instead of deriving a new one
- [ ] This ensures the full chain daemon → agent → child agent shares the same principal identity
...
- [ ] In `terminate_session()`, call `db.revoke_session_tokens(session_id)` early in the cleanup sequence
- [ ] This ensures any subsequent CLI calls with the token are rejected, regardless of how the session ended (API, UI adapter, inactivity, stale detection)
...
- [ ] Return format conveys the resolved principal, for example `"Principal: human:<email>"` or `"Principal: system:<stable-id>"`
...
- [ ] Test `issue_session_token()` creates a valid record with role
...
- [ ] Test principal resolution: human email → `human:<email>` with role, no email → `system:<stable-id>` with admin role and no truncated identifier leakage
...
- [ ] Test `CallerIdentity.principal_role` is populated from token record
...
- [ ] Test `is_command_allowed()` permits calls when principal is present and human_role is None
- [ ] Test `is_command_allowed()` uses `principal_role` for the human-role allowlist check when `human_role` is absent
- [ ] Test `is_command_allowed()` still denies when both principal and human_role are None
...
- [ ] Test child session inherits parent's principal via channel_metadata
- [ ] Test child session receives its own token with the inherited principal
- [ ] Test principal chain: daemon → agent → child agent all share the same principal
...
- [ ] Test a token-authenticated agent session can complete `telec sessions send` with the expected role
- [ ] Test `telec auth whoami` resolves the principal inside an agent session without asserting on full human-facing prose
...
- [ ] Run the repository's pre-commit hooks as the primary verification path
- [ ] Run targeted unit/integration tests for the touched auth/session files during RED/GREEN; broaden only if hooks fail or targeted runs are inconclusive
- [ ] Run lint/type-check commands directly only when isolating a hook failure or when a required check is not covered by hooks
...
- [ ] Update `docs/project/spec/identity-and-auth.md` (and any touched CLI/README surface) so the token-auth path and `whoami` behavior match the delivered code
- [ ] Update `todos/agent-session-auth/demo.md` with an executable flow that shows principal resolution and an agent-initiated command succeeding before revocation
