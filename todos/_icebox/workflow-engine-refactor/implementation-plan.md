...
**Changes from prior plan review (C1/C2/I1)**:

- **C1 resolved — language-aware code steps included.** The prior plan deferred
  language baselines. This revision includes them: the engine detects
  `produces_code: true` on a step, reads the project language from
  `teleclaude.yml`, and loads the appropriate language baseline required reads.
  New config key `language` added to `teleclaude.yml`. New `languages/` baseline
  files created. This covers requirements item 5 fully.

- **C1 resolved — consolidated dispatch surface is the engine module.** The
  requirement asks for "one workflow-oriented dispatch surface" — that is the
  engine module itself. `next_prepare()` and `next_work()` delegate routing to
  the engine, which resolves steps from YAML definitions. The consolidation is
  internal (one engine, many workflows). The existing CLI entry points are
  preserved unchanged per requirement item 7. A user-facing `/next-workflow`
  command shortcut is deferred to a follow-up todo — it is a convenience
  wrapper, not the dispatch surface.

- **C2 resolved — schema expanded for all work-machine behaviors.** The step
  schema now includes `alt_commands` (bug-variant dispatch), `pre_dispatch`
  (action templates before worker dispatch), `validators` (list of named
  validators per step), and `threshold` (gate-step score checks). The validator
  protocol uses adapter wrappers matching existing function signatures:
  `run_build_gates(worktree_cwd, slug)` and
  `verify_artifacts(worktree_cwd, slug, phase, *, is_bug)` are wrapped behind
  a uniform `ValidatorFn(worktree_cwd, slug, phase, is_bug) -> (bool, str)`
  interface.

- **I1 resolved — pre-commit hooks in final verification.** Task 5.2 now
  includes explicit pre-commit hook verification via `git commit --dry-run` or
  the existing hook invocation path.
...
The schema must express every behavior the current prepare and work machines
exhibit — including bug-variant dispatch, pre-dispatch actions, multi-validator
gate steps, and score thresholds. Missing schema fields force the builder to
invent workarounds during implementation.

- [ ] Define `StepInput` dataclass: `step: str`, `artifacts: list[str]`
- [ ] Define `ProducerConfig` dataclass:
  - `command: str` — the `SlashCommand` value to dispatch
  - `required_reads: list[str]` — doc snippet IDs for worker context
  - `artifacts: list[str]` — glob patterns for expected output files
  - `inputs: list[StepInput]` — artifacts from prior steps
  - `thinking_mode: str` — `slow`/`med`/`fast` for dispatch
  - `alt_commands: dict[str, str]` — condition-keyed alternate commands
    (e.g. `{"bug": "next-bugs-fix"}` for the build step's bug variant at
    core.py:4050-4063). Empty dict when no variants exist.
  - `pre_dispatch: str | None` — action template executed before worker
    dispatch (e.g. `"telec todo mark-phase {slug} --phase build --status
    started"` matching core.py:4048). `{slug}` is substituted at runtime.
    `None` when no pre-dispatch action is needed.
- [ ] Define `ReviewerConfig` dataclass: `required_reads: list[str]`,
      `artifacts: list[str]`, `thinking_mode: str`, `command: str`
- [ ] Define `StepEvents` dataclass: `produced: str | None`,
      `approved: str | None`
- [ ] Define `StepDefinition` dataclass:
  - `name: str` — step identifier
  - `producer: ProducerConfig | None` — `None` for mechanical-only steps
    (grounding_check, gates)
  - `reviewer: ReviewerConfig | None` — if present, engine runs
    produce-review-fix loop
  - `max_rounds: int` — review iteration cap (default
    `DEFAULT_MAX_REVIEW_ROUNDS`)
  - `human_gate: bool` — pause for human confirmation after approval
  - `needs_worktree: bool` — engine ensures worktree before dispatch
  - `produces_code: bool` — engine loads language baseline required_reads
  - `state_key: str` — field in `state.yaml` the engine reads/writes
  - `events: StepEvents` — lifecycle events at transitions
  - `validators: list[str]` — names of validators to run at this step
    (e.g. `["build_gates", "artifact_verification"]` for the gates step
    matching core.py:4079 and core.py:4096). Empty list when no
    validation. Multiple validators run in sequence; first failure stops
    the chain.
  - `threshold: dict[str, int] | None` — gate threshold checks
    (e.g. `{"dor_score": 8}` for the DOR gate step matching
    `DOR_READY_THRESHOLD` at core.py:90). `None` when no threshold.
...
- [ ] Implement `load_workflow(path: Path) -> WorkflowDefinition` that parses
      ...
      with descriptive messages for missing or invalid fields. Accept missing
      optional fields (alt_commands defaults to `{}`, validators defaults to
      `[]`, threshold defaults to `None`, pre_dispatch defaults to `None`).
...
- [ ] Step `gate`: producer command = `SlashCommand.NEXT_PREPARE_GATE`,
      no reviewer, state_key = `dor`, threshold = `{"dor_score": 8}`
- [ ] Step `grounding_check`: no producer (mechanical check), state_key =
      `grounding`, no dispatch
- [ ] Step `re_grounding`: producer command =
      `SlashCommand.NEXT_PREPARE_DRAFT`, resets plan review verdict
- [ ] Steps `prepared` and `blocked`: terminal states, no dispatch

**`workflows/work.yaml`** — transcription of `next_work()` routing logic
(core.py:3886-4200):

- [ ] Step `build`: producer command = `SlashCommand.NEXT_BUILD`,
      alt_commands = `{"bug": "next-bugs-fix"}` (matching core.py:4050-4063),
      pre_dispatch = `"telec todo mark-phase {slug} --phase build --status
      started"` (matching core.py:4048), needs_worktree = true,
      produces_code = true
- [ ] Step `gates`: no producer (mechanical validation),
      validators = `["build_gates", "artifact_verification"]`
      (matching core.py:4079 and core.py:4096), state_key = `build`
- [ ] Step `review`: producer command = `SlashCommand.NEXT_REVIEW_BUILD`,
      max_rounds from state (`max_review_rounds`), state_key = `review`
...
- [ ] Step `finalize`: producer command = `SlashCommand.NEXT_FINALIZE`,
      needs_worktree = true, finalize handoff logic for
      `ready` → `handed_off` transition
...
equivalence. Each step's `command`, `alt_commands`, `pre_dispatch`, `state_key`,
`artifacts`, `events`, `validators`, `threshold`, and `max_rounds` must match
the current handler's behavior.

### Task 2.3: Language baseline infrastructure

**File(s):** `languages/python/baseline.md`, `languages/typescript/baseline.md`,
`teleclaude.yml`, `teleclaude/core/next_machine/engine.py`

**Why**: Requirements item 5 specifies that code-producing steps augment their
context with language-specific conventions. The engine needs a way to detect the
project language and load the corresponding baseline. This keeps the engine
language-agnostic — language details live in external baseline files, not in
Python code. The config key provides a stable detection mechanism that
`telec init` can set during project onboarding.

- [ ] Create `languages/python/baseline.md` with:
  - Test framework: pytest
  - RED marker syntax: `@pytest.mark.xfail(strict=True)`
  - Spec file pattern: `tests/**/test_*.py`
  - Test command: `make test` / `pytest`
  - Lint command: `make lint`
  - Type check: pyright, mypy
- [ ] Create `languages/typescript/baseline.md` with:
  - Test framework: vitest
  - RED marker syntax: `test.fails()`
  - Spec file pattern: `**/*.spec.ts`
  - Test command: `npm test` / `vitest`
  - Lint command: `npm run lint`
  - Type check: tsc
- [ ] Add `language: str | None` field to the project config model in
      `teleclaude/config/__init__.py` (alongside existing `project_name`,
      `business`, etc.). Default to `None` (language detection disabled).
      Read from `teleclaude.yml` top-level `language` key.
- [ ] Update `teleclaude.yml` in this repo: add `language: python`
- [ ] Update config spec (`docs/project/spec/teleclaude-config.md`): add
      `language` key to the machine-readable surface
- [ ] Implement `load_language_baseline(language: str) -> list[str]` in
      `engine.py` that reads `languages/{language}/baseline.md` and returns
      the content as additional required_reads context. Returns empty list if
      file not found (graceful degradation for unknown languages).
- [ ] In `emit_dispatch()`: when step has `produces_code: true` and the
      project config has a `language` set, call `load_language_baseline()` and
      merge the result into the dispatch instruction's required_reads

**Verification**: Unit tests:
- `load_language_baseline("python")` returns non-empty content from baseline file
- `load_language_baseline("unknown")` returns empty list without error
- Dispatch emission for a `produces_code: true` step includes language baseline
  when project language is set
- Dispatch emission for a `produces_code: true` step without project language
  omits language baseline (graceful degradation)
...
- [ ] Handle review round limit: if rounds exceed step's `max_rounds`, action
      is `block` — same as current BLOCKED transition
...
- [ ] Handle threshold check: for gate steps with `threshold` metadata,
      verify score meets threshold (e.g. DOR score >= 8 at core.py:3243-3251)
...
- [ ] Handle bug variant: when `kind=bug` in state and step has
      `alt_commands.bug`, use the alternate command instead of the primary
      `producer.command` — same as core.py:4050-4063
- [ ] Handle pre_dispatch: when step has `producer.pre_dispatch`, substitute
      `{slug}` and include in the `format_tool_call()` `pre_dispatch` argument
      — same as core.py:4048
- [ ] Handle gate validation: when step has `validators`, run each through
      the validator registry (Task 3.3). First failure stops the chain and
      returns the gate failure output — same as core.py:4075-4111
...
- [ ] Implement `emit_dispatch(step, action, slug, cwd, worktree_cwd, db, ...) -> str`
  ...
  - `command` from step's producer or reviewer `command` field, or from
    `alt_commands` when a variant condition matches
  - `args` = slug
  - `project` = cwd
  ...
  - `pre_dispatch` from step's `producer.pre_dispatch` with `{slug}`
    substitution, or empty when `None`
...
- [ ] When step has `produces_code: true` and project language is configured,
      include language baseline content in the dispatch instruction's context
      (via `load_language_baseline()` from Task 2.3)
...
lets steps declare their validators by name in YAML, and the engine looks them up
at runtime. This decouples validation logic from step routing without duplicating
the validation code — the registry references existing functions through adapter
wrappers.

- [ ] Define validator protocol:
      `ValidatorFn = Callable[[str, str, str, bool], tuple[bool, str]]`
      taking `(worktree_cwd, slug, phase, is_bug)` and returning
      `(passed, output)`. This uniform signature accommodates both
      `run_build_gates(worktree_cwd, slug)` (which ignores `phase` and
      `is_bug`) and `verify_artifacts(worktree_cwd, slug, phase, *,
      is_bug=False)` (which uses all four).
- [ ] Create adapter wrappers for existing functions:
  - `_build_gates_adapter(worktree_cwd, slug, phase, is_bug)` →
    calls `run_build_gates(worktree_cwd, slug)`, ignores `phase`/`is_bug`
  - `_artifact_verification_adapter(worktree_cwd, slug, phase, is_bug)` →
    calls `verify_artifacts(worktree_cwd, slug, phase, is_bug=is_bug)`
- [ ] Create registry dict `VALIDATOR_REGISTRY: dict[str, ValidatorFn]`
      mapping names to adapter callables:
  - `"build_gates"` → `_build_gates_adapter`
  - `"artifact_verification"` → `_artifact_verification_adapter`
- [ ] Implement `run_step_validators(step, worktree_cwd, slug, phase, is_bug)
      -> tuple[bool, str]` that:
  - Iterates `step.validators` in order
  - Looks up each name in `VALIDATOR_REGISTRY` (raises `KeyError` with
    descriptive message for unknown names)
  - Runs validators in sequence — first failure stops the chain and returns
    the combined failure output
  - Returns `(True, "")` if all validators pass or list is empty
- [ ] Keep `run_build_gates()` and `verify_artifacts()` functions in `core.py`
      — the adapters import and delegate to them. Import paths:
      `from teleclaude.core.next_machine.core import run_build_gates,
      verify_artifacts`

**Verification**: Unit tests:
- Registry lookup for `"build_gates"` returns a callable
- Registry lookup for `"artifact_verification"` returns a callable
- Unknown validator name raises `KeyError` with descriptive message
- `run_step_validators()` with two validators runs both in sequence
- `run_step_validators()` stops at first failure and returns failure output
- `run_step_validators()` with empty list returns `(True, "")`
- `_build_gates_adapter` correctly delegates to `run_build_gates`
- `_artifact_verification_adapter` correctly passes `phase` and `is_bug` to
  `verify_artifacts`
...
- [ ] Preserve bug route (core.py:4050-4063) — engine reads `kind=bug` from
      state and uses step's `alt_commands["bug"]` for the alternate command
- [ ] Preserve pre_dispatch behavior (core.py:4048) — engine reads step's
      `producer.pre_dispatch` and substitutes `{slug}`
...
- [ ] Verify pre-commit hooks pass: stage all changes and run
      `git diff --cached --name-only | head -1 && git stash && git stash pop`
      is not viable (stash forbidden); instead confirm via
      `make test && make lint` (the pre-commit hooks invoke these) and verify
      no hook-only checks are missed by running the actual commit at the end
      of the build
...
- [ ] Verify language baseline loading works for `produces_code: true` steps
      when `language: python` is set in `teleclaude.yml`
...
- `teleclaude/core/next_machine/__init__.py` — Update exports if needed
- `teleclaude/config/__init__.py` — Add `language` config field
- `teleclaude.yml` — Add `language: python` key

### Created
- `teleclaude/core/next_machine/engine.py` — Workflow engine: types, loader,
  step resolution, dispatch emission, language baseline loading
- `teleclaude/core/next_machine/validators.py` — Named validator registry
  with adapter wrappers for existing validation functions
- `workflows/prepare.yaml` — Prepare workflow definition
- `workflows/work.yaml` — Work workflow definition
- `languages/python/baseline.md` — Python language baseline (test framework,
  RED markers, spec patterns, commands)
...
- `docs/project/spec/teleclaude-config.md` — Add `language` key to config
  surface

## Deferred to Follow-up Todo

- **Consolidated `/next-workflow` CLI command**: A user-facing command that
  maps positional args (`/next-workflow prepare discovery`) to engine step
  dispatch. Requires a new agent command artifact and `SlashCommand` enum
  entry. This is a convenience wrapper over the engine — the dispatch surface
  itself (the engine module) is delivered by this refactor.
