...
- [ ] Refactor Redis/MCP entry to use the mapper and dispatch normalized commands.
- [ ] Remove transport-specific branching in core handlers for REST/Redis.

## Group 3 — Handler Signature Unification
- [ ] Update handler entry points to accept normalized command models (or a single normalized request object) instead of loose payload/metadata.
- [ ] Ensure ordering-sensitive paths (session creation welcome/output) are still awaited.

## Group 4 — Tests + Verification
- [ ] Update existing tests that depended on legacy payload/metadata shapes.
- [ ] Add regression tests for session creation and agent start to assert unchanged behavior.
- [ ] Run format/lint/tests and commit per task.
