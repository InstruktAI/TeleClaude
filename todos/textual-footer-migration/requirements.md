...
- Remove `CursorContextChanged` message and its handler chain in `app.py`. Textual's Footer auto-switches bindings when focus moves between views. The old sessions cursor-context (different hints per item type) is simplified: all session bindings are always visible. This aligns with the "keep everything visible" user requirement and eliminates per-item-type hint maintenance.
