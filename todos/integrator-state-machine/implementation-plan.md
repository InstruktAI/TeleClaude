...
- [x] Define `IntegrationPhase` enum: `IDLE`, `LEASE_ACQUIRED`, `CANDIDATE_DEQUEUED`,
...
- [x] Define `IntegrationCheckpoint` frozen dataclass: phase, candidate_key
...
- [x] Implement atomic checkpoint read/write (JSON, temp file + os.replace — same
      pattern as `IntegrationQueue`)
- [x] Implement checkpoint recovery: if phase is mid-candidate and lease is stale,
...
- [x] Implement `async def next_integrate(db: Db, slug: str | None, cwd: str, caller_session_id: str | None = None) -> str`
- [x] Read checkpoint; if IDLE, acquire lease via `IntegrationLeaseStore`
- [x] If lease acquired, pop next READY candidate via `IntegrationQueue.pop_next()`
- [x] If slug provided and doesn't match next in queue, return error (queue is FIFO)
- [x] Route to per-phase handler based on checkpoint phase
- [x] Each handler: execute deterministic block, update checkpoint, return instruction block
- [x] Use `format_tool_call()` / `format_error()` patterns from `next_work()` for output
- [x] Log phase transitions: `NEXT_INTEGRATE_PHASE slug=X phase=Y decision=Z duration_ms=N`

### Task 1.3: Implement per-phase handlers

**File(s):** `teleclaude/core/integration/state_machine.py`

- [x] **LEASE_ACQUIRED → CANDIDATE_DEQUEUED**: Pop candidate, emit `integration.candidate.dequeued`,
      write checkpoint, continue to clearance
- [x] **CANDIDATE_DEQUEUED → CLEARANCE_WAIT**: Call `MainBranchClearanceProbe.check()`,
      if not cleared return wait instruction with blocking sessions and retry guidance,
      if cleared continue to merge
- [x] **CLEARANCE_WAIT → MERGE_CLEAN/MERGE_CONFLICTED**: `git fetch origin`,
...
- [x] **MERGE_CLEAN decision point returns:** diff stats, branch commit log
...
- [x] **MERGE_CONFLICTED decision point returns:** conflicted file list, branch context —
      agent resolves conflicts, stages, composes commit message, runs `git commit`
- [x] **AWAITING_COMMIT → COMMITTED**: Detect commit — if HEAD advanced past
      ...
      conflict files
- [x] **COMMITTED → DELIVERY_BOOKKEEPING**: `telec roadmap deliver {slug}` (skip for
      bugs), `telec todo demo create {slug}` if demo exists, stage and commit
      delivery files, emit `integration.candidate.committed`
- [x] **DELIVERY_BOOKKEEPING → PUSH_SUCCEEDED/PUSH_REJECTED**: `git push origin main`.
...
- [x] **PUSH_SUCCEEDED → CLEANUP**: Remove worktree, delete local and remote branch,
      remove todo directory, commit cleanup, emit `integration.candidate.delivered`,
      run `make restart`
- [x] **CLEANUP → next candidate**: Mark queue item integrated, increment items_processed,
      reset phase to pop next candidate. If queue empty → COMPLETED
- [x] **COMPLETED**: Release lease, emit `integration.completed` with summary
      (candidates processed, candidates blocked, total duration), return exit instruction
- [x] **Blocked path** (from MERGE_CONFLICTED when agent signals unresolvable): emit
...
- [x] After agent recovers from push rejection (pull, rebase, push), next
...
- [x] Add lifecycle event type definitions alongside existing readiness events:
...
- [x] Define payload schemas per event (per input.md lifecycle events table)
- [x] All events carry integrator `session_id` as source
...
- [x] Add emit helpers in `integration_bridge.py` for new lifecycle event types
      (extend existing `emit_deployment_*` pattern)
- [x] Call emit at each state transition in the phase handlers
- [x] Map existing bridge events to lifecycle events: `emit_deployment_completed` →
...
- [x] Add route handler mirroring `todo_work()`:
      ```python
      ...
      ```
- [x] Wire `next_integrate` import from `teleclaude.core.integration.state_machine`

### Task 3.2: Add CLI command `telec todo integrate [<slug>]`

**File(s):** `teleclaude/cli/tool_commands.py`, `teleclaude/cli/telec.py`

- [x] Add `handle_todo_integrate(args)` mirroring `handle_todo_work()`
- [x] Call `tool_api_call("POST", "/todos/integrate", json_body={"cwd": cwd, "slug": slug})`
- [x] Register in CLI surface (`CLI_SURFACE` dict and `CommandDef` entries in `telec.py`)
- [x] Parse optional slug argument
...
- [x] Replace prose integration steps with a loop calling `telec todo integrate`
- [x] Document the three decision points and what the agent should do at each:
...
- [x] The command becomes: call `telec todo integrate`, read instruction block, execute
      decision, call again, repeat until exit instruction. On exit: self-end session
- [x] Preserve behavioral guidance (commit message quality, conflict resolution approach)
...
- [x] Unit tests for checkpoint read/write/recovery (atomic writes, stale lease recovery)
- [x] Unit tests for each phase handler (mock git operations and primitives)
- [x] Integration test: full candidate lifecycle (merge → commit → push → cleanup)
- [x] Test idempotency: call at same checkpoint state produces same output
- [x] Test crash recovery: interrupt mid-phase, verify re-entry resumes correctly
- [x] Test queue drain: multiple candidates processed in FIFO order
- [x] Test blocked candidate: conflict → follow-up creation → skip to next
- [x] Test clearance wait: probe returns not-cleared, verify wait instruction
- [x] Test push rejection: verify decision point returns, verify recovery detection
- [x] Run `make test`

### Task 5.2: Quality Checks

- [x] Run `make lint`
      Note: `make lint` exits non-zero due to pre-existing pylint issues (cyclic imports
      in `teleclaude.cli.telec`, duplicate-code, score 9.40/10). These existed before
      this build (score was 9.39/10 on HEAD). No new pylint errors introduced.
      Guardrails, ruff, pyright, markdown validation, and resource validation all pass.
