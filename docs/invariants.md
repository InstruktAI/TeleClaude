...
## Next Machine (Orchestration)
- Next Machine is stateless; all state is derived from project files (roadmap, requirements, plans, state.json).
- `next_prepare` (Phase A) is always interactive/HITL unless explicitly told otherwise.
- `next_work` (Phase B) is deterministic and autonomous.
- Dependency gating in `next_work` is mandatory; blocked items are never claimed.
- Files checked by Next Machine (requirements, plans) must be tracked by git to ensure worktree accessibility.

## Agent Events
- `AgentCoordinator` is the single source of truth for routing agent lifecycle events.
- Agent prompts always clear session notification flags.
- Agent stop events are debounced to handle redundant triggers from some agents.
- Session titles are updated with agent info (agent name, thinking mode) only after agent stabilization.
