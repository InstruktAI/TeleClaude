...
1. Run `telec todo demo {slug}` — all code blocks must exit 0.
2. If validation fails, the build is not done. Fix forward.
3. Create `demos/{slug}/` folder with `snapshot.json` and `demo.md`.
...
### CLI validation (telec todo demo)

- **No slug:** lists all available demos (table format: slug, title, version, delivered date).
- **With slug:** extracts fenced bash code blocks from `demo.md`, executes them sequentially. All must exit 0. Falls back to `snapshot.json` `demo` field for backward compatibility.
