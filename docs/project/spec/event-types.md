...
- `agent_event`: agent lifecycle wrapper with payload types (`session_start`, `user_prompt_submit`, `after_model`, `agent_output`, `agent_stop`, `notification`, `session_end`, `error`).
...
- `agent_event` payload types: `session_start`, `user_prompt_submit`, `after_model`, `agent_output`, `agent_stop`, `notification`, `session_end`, `error`.
...
- `session_end` is part of the normalized type universe, but the hook receiver currently forwards only handled events (`session_start`, `user_prompt_submit`, `after_model`, `agent_output`, `agent_stop`, `notification`, `error`) into `hook_outbox`.
