...
- `agent_event`: agent lifecycle wrapper with payload types (`session_start`, `user_prompt_submit`, `agent_stop`, `notification`, `session_end`, `error`).
...
- `agent_event` payload types: `session_start`, `user_prompt_submit`, `agent_stop`, `notification`, `session_end`, `error`.
