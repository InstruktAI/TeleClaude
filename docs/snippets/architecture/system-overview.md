...
- Transport adapters: Redis Streams for cross-computer requests and output streams.
...
Primary flows
- Ingress from Telegram/MCP/API -> AdapterClient -> daemon command pipeline -> tmux -> output polling -> AdapterClient -> UI/transport.
- Remote requests use transport adapters and stream output back via Redis.
